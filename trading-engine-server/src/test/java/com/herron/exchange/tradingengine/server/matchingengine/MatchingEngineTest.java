...
class MatchingEngineTest {
    ...
    void test_handle_orderbook_data_status_request_ok() {
        var orderbookData = new HerronOrderbookData("1", "1", MatchingAlgorithmEnum.PRO_RATA, "eur", 0, 1);
    ...
    void test_handle_orderbook_data_status_request_error_invalid_matching_algo() {
        var orderbookData = new HerronOrderbookData("1", "1", MatchingAlgorithmEnum.INVALID_MATCHING_ALGORITHM, "eur", 0, 1);
        var request = new HerronOrderbookDataRequest(1, orderbookData);
    ...
    void test_handle_stateChange_request_ok_multiple_changes() {
        ...
        var stateChange2 = new HerronStateChange("1", StateChangeTypeEnum.CONTINUOUS_TRADING, 1);
    ...
    void test_handle_order_request_ok() {
        var orderbookData = new HerronOrderbookData("orderbookId", "1", MatchingAlgorithmEnum.PRO_RATA, "eur", 0, 1);
        var orderbookDataRequest = new HerronOrderbookDataRequest(1, orderbookData);
        matchingEngine.handleMessage(orderbookDataRequest);

        var stateChange = new HerronStateChange("orderbookId", StateChangeTypeEnum.AUCTION_TRADING, 1);
        var stateChangeRequest = new HerronStateChangeRequest(1, stateChange);
        ...
        assertEquals(1, matchingEngine.getOrderQueueSize());
    }

    @Test
    void test_handle_order_request_error_no_state() {
        var orderbookData = new HerronOrderbookData("orderbookId", "1", MatchingAlgorithmEnum.PRO_RATA, "eur", 0, 1);
        var orderbookDataRequest = new HerronOrderbookDataRequest(1, orderbookData);
        matchingEngine.handleMessage(orderbookDataRequest);
    ...
    void test_handle_order_request_error_trade_stop() {
        ...
        var stateChange = new HerronStateChange("orderbookId", StateChangeTypeEnum.CONTINUOUS_TRADING, 1);
        var stateChangeRequest = new HerronStateChangeRequest(1, stateChange);
        matchingEngine.handleMessage(stateChangeRequest);
